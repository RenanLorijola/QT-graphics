...
void OpenGLWidget::loadTextureLayer(std::shared_ptr<Model> m)
{
    ...
    if (m->textureIDLayerLower)
        glDeleteTextures(1, &m->textureIDLayerLower);
    ...
    if (m->textureIDLayerUpper)
        glDeleteTextures(1, &m->textureIDLayerUpper);
...
void OpenGLWidget::loadEarthTexture(std::shared_ptr<Model> m)
{
    ...
    if (m->textureIDEarth)
        glDeleteTextures(1, &m->textureIDEarth);

    glGenTextures(1, &m->textureIDEarth);
    glBindTexture(GL_TEXTURE_2D, m->textureIDEarth);
    ...
    if (m->textureIDCloud)
        glDeleteTextures(1, &m->textureIDCloud);

    glGenTextures(1, &m->textureIDCloud);
    glBindTexture(GL_TEXTURE_2D, m->textureIDCloud);
...
void OpenGLWidget::loadNormalMapTexture(std::shared_ptr<Model> m)
{
    ...
    if (m->textureIDNormalMapColor)
        glDeleteTextures(1, &m->textureIDNormalMapColor);
    ...
    if (m->textureIDNormalMapNormals)
        glDeleteTextures(1, &m->textureIDNormalMapNormals);
...
void OpenGLWidget::loadCubeMapTexture(std::shared_ptr<Model> m)
{
    ...
    if (m->textureIDCubeMap)
        glDeleteTextures(1, &m->textureIDCubeMap);
